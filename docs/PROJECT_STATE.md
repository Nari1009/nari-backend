...
- No Wompi calls, Wompi configuration, Client/Admin changes, checkout-connected reservation/release, payment webhook processing or commercial effects were added. The R12B payment migration independently enables RLS and revokes `anon`/`authenticated` in the same transaction.
- **R12C:** Reservation foundation COMPLETE / VERIFIED IN DEV. R12 overall remains IN PROGRESS / DEV ONLY; checkout/payment-attempt integration is not implemented.

## R12C — INVENTORY RESERVATION FOUNDATION (DEV ONLY)

- **Status:** IN PROGRESS locally. Added an isolated reservation domain foundation with `stock_reservations`, `stock_reservation_items`, canonical `ACTIVE`/`COMMITTED`/`RELEASED`/`EXPIRED` states, transactional reserve/commit/release/expiration primitives and deterministic inventory-movement references.
- `products.stock` remains available-to-sell inventory: reservation decreases it, release/expiration restores it, and sale commit records the sale and increments `soldCount` without decrementing stock again.
- The migration has been applied and structurally verified in Supabase DEV: 26/26 public tables have RLS enabled, 0 have RLS disabled, FORCE RLS is 0, reservation tables were empty at verification, and the reservation indexes, foreign keys and checks are present. Real PostgreSQL validation A–E and corrected F1/F2 passed with cleanup verified. The service is not connected to current checkout/order creation. Client, Admin, legacy payment webhook and Wompi remain unchanged; PROD was not accessed.
- DEV preflight for the movement-reference index was manually verified: 67 historical inventory movements, 0 non-null references and 0 duplicate non-null references. Reservation-finalized `sale` movements intentionally use quantity `0`; legacy historical movements are not rewritten.
