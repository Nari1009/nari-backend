...
## R11 PAUSED / DEV ONLY — 2026-09-20

The store launch is now the priority, so R11 development is paused. The local Conversational Agent Proof is preserved and validated with 231 Backend tests and 7 pilot tests, but it is not enabled or deployed. `NARI_AI_AGENT_ENABLED` defaults to `false`; Render DEV configuration and real-provider smoke QA remain pending.
