...
router.post('/abandoned', async (req, res, next) => {
  try {
    ...
    let cart = await get('SELECT id FROM abandoned_carts WHERE email = ? AND convertedAt IS NULL ORDER BY updatedAt DESC LIMIT 1', [email]);
    if (cart) {
...
router.get('/abandoned/:id', async (req, res, next) => {
  try {
    const cart = await get('SELECT id, items FROM abandoned_carts WHERE id = ? AND convertedAt IS NULL', [req.params.id]);
