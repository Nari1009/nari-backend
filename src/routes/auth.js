...
router.get('/orders', requireUser, async (req, res, next) => {
  ...
  const orders = await all('SELECT id, ordernumber AS "orderNumber", createdat AS date, status, deliveredat AS "deliveredAt", total, shippingprovider AS "shippingProvider", trackingnumber AS "trackingNumber", customeremailsnapshot AS "customerEmailSnapshot", customerfirstnamesnapshot AS "customerFirstNameSnapshot", customerlastnamesnapshot AS "customerLastNameSnapshot", customerphonesnapshot AS "customerPhoneSnapshot" FROM orders WHERE customerid = ? ORDER BY createdat DESC', [customer.id]);
...
router.get('/orders/:id', requireUser, async (req, res, next) => {
  try {
    ...
    const order = await get(`SELECT id, ordernumber AS "orderNumber", createdat AS date, status, deliveredat AS "deliveredAt", total, subtotal, shippingtotal AS "shippingTotal", discounttotal AS "discountTotal", shippingzone AS "shippingZone", deliverytype AS "deliveryType", samedayeligible AS "sameDayEligible", shippingpolicyversion AS "shippingPolicyVersion", shippingprovider AS "shippingProvider", trackingnumber AS "trackingNumber", shippingaddress AS "shippingAddress", customeremailsnapshot AS "customerEmailSnapshot", customerfirstnamesnapshot AS "customerFirstNameSnapshot", customerlastnamesnapshot AS "customerLastNameSnapshot", customerphonesnapshot AS "customerPhoneSnapshot", documenttypesnapshot AS "documentTypeSnapshot", documentnumbersnapshot AS "documentNumberSnapshot"
