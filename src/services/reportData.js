...
async function getReportData({ from, to, lowStockThreshold = 3 }) {
  ...
  const [orders, items, inventory, movements, customers, totals, financial, abandonedCarts] = await Promise.all([
    ...
    all(`SELECT oi.orderId, o.createdAt, o.status, oi.productId, oi.productName, oi.quantity, oi.unitPrice, oi.unitCost, p.sku, p.brand, p.category, p.cost AS currentCost, p.price AS currentPrice, c.firstName, c.lastName, c.email, (SELECT city FROM account_addresses WHERE userId = o.userId AND isDefault = 1 ORDER BY updatedAt DESC LIMIT 1) AS city, (SELECT department FROM account_addresses WHERE userId = o.userId AND isDefault = 1 ORDER BY updatedAt DESC LIMIT 1) AS department
    ...
    all('SELECT id, sku, name, brand, category, stock, minimumStock, cost, price, status FROM products ORDER BY brand, name'),
  ...
  for (const item of items) {
    ...
    const current = productGroups.get(key) || { productId: item.productId, sku: item.sku || '', productName: item.productName, brand: item.brand || '', category: item.category || '', units: 0, orders: new Set(), grossSales: 0, discounts: 0, sales: 0, cost: 0, profit: 0 };
