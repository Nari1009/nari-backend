...
const keyAliases = { createdat: 'createdAt', customerid: 'customerId', shippingtotal: 'shippingTotal', discounttotal: 'discountTotal', paymentfee: 'paymentFee', shippingcost: 'shippingCost', refundedtotal: 'refundedTotal', shippingprovider: 'shippingProvider', trackingnumber: 'trackingNumber', shippingaddress: 'shippingAddress', customeremailsnapshot: 'customerEmailSnapshot', customerfirstnamesnapshot: 'customerFirstNameSnapshot', customerlastnamesnapshot: 'customerLastNameSnapshot', customerphonesnapshot: 'customerPhoneSnapshot', authuserid: 'authUserId', customerstatus: 'customerStatus', firstname: 'firstName', lastname: 'lastName', orderid: 'orderId', productid: 'productId', productname: 'productName', unitprice: 'unitPrice', unitcost: 'unitCost', currentcost: 'currentCost', currentprice: 'currentPrice', minimumstock: 'minimumStock', stockbefore: 'stockBefore', stockafter: 'stockAfter', purchaseid: 'purchaseId', performedby: 'performedBy', firstpurchaseat: 'firstPurchaseAt', lastpurchaseat: 'lastPurchaseAt', ordercount: 'orderCount', totalpurchased: 'totalPurchased', grosssales: 'grossSales', paymentfees: 'paymentFees', productcost: 'productCost', salevalue: 'saleValue', costvalue: 'costValue' };
const normalizeRows = (rows) => rows.map((row) => Object.fromEntries(Object.entries(row).map(([key, value]) => [keyAliases[key] || key, value])));

async function getReportData({ from, to, lowStockThreshold = 3 }) {
  const orderParams = [...validSaleStatuses, from, to];
  let [orders, items, inventory, movements, customers, totals, financial, abandonedCarts] = await Promise.all([
    all(`SELECT o.id, o.customerId, o.createdAt, o.status, o.total, o.subtotal, o.shippingTotal, o.discountTotal, o.paymentFee, o.shippingCost, o.refundedTotal, o.shippingProvider, o.trackingNumber, o.shippingAddress, o.customerEmailSnapshot, o.customerFirstNameSnapshot, o.customerLastNameSnapshot, o.customerPhoneSnapshot, c.authUserId, c.status AS customerStatus, c.firstName, c.lastName, c.email, c.phone, (SELECT city FROM account_addresses WHERE userId = o.userId AND isDefault = 1 ORDER BY updatedAt DESC LIMIT 1) AS city, (SELECT department FROM account_addresses WHERE userId = o.userId AND isDefault = 1 ORDER BY updatedAt DESC LIMIT 1) AS department, (SELECT country FROM account_addresses WHERE userId = o.userId AND isDefault = 1 ORDER BY updatedAt DESC LIMIT 1) AS country
    ...
    all(`SELECT oi.orderId, o.createdAt, o.status, oi.productId, oi.productName, oi.quantity, oi.unitPrice, oi.unitCost, p.sku, p.brand, p.category, p.supplier, p.cost AS currentCost, p.price AS currentPrice, o.customerEmailSnapshot, o.customerFirstNameSnapshot, o.customerLastNameSnapshot, c.authUserId, c.firstName, c.lastName, c.email, (SELECT city FROM account_addresses WHERE userId = o.userId AND isDefault = 1 ORDER BY updatedAt DESC LIMIT 1) AS city, (SELECT department FROM account_addresses WHERE userId = o.userId AND isDefault = 1 ORDER BY updatedAt DESC LIMIT 1) AS department
    ...
    all(`SELECT c.id, c.authUserId, c.status, c.firstName, c.lastName, c.email, c.phone, c.city, c.department, c.firstPurchaseAt, c.lastPurchaseAt, c.orderCount, c.totalPurchased FROM customers c ORDER BY c.totalPurchased DESC`),
    ...
    get("SELECT COUNT(*) AS count FROM abandoned_carts WHERE convertedAt IS NULL OR trim(CAST(convertedAt AS TEXT)) = ''"),
  ]);
  orders = normalizeRows(orders); items = normalizeRows(items); inventory = normalizeRows(inventory); movements = normalizeRows(movements); customers = normalizeRows(customers); totals = normalizeRows([totals])[0]; financial = normalizeRows([financial])[0];
