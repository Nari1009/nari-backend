...
const createPoint10Service = ({ resolver, candidateService, finalProductRepository } = {}) => ({
  ...
  async budgetRoutine({ request, interpretation }) {
    ...
    if (!validBudget(budget)) return safeAnswer(interpretation.intent, interpretation.profile, 'Necesito un presupuesto total positivo en COP para construir la rutina.', { budget: { limit: null, total: null, currency: 'COP', withinBudget: false } });
    const plan = createRoutinePlan(interpretation.profile);
    if (!plan) return genericFollowUp(interpretation.intent, interpretation.profile, '¿Qué tipo de piel tienes o qué objetivo quieres priorizar para construir una rutina dentro de tu presupuesto?');
    ...
    for (const step of planSteps(plan)) {
      if (ownedSteps.has(step)) continue;
    ...
    if (requiredSteps.some((step) => groups[step].length === 0)) return safeAnswer(interpretation.intent, interpretation.profile, 'No pude confirmar una rutina completa dentro del catálogo actual y el presupuesto indicado.', { budget: { limit: budget, total: null, currency: 'COP', withinBudget: false } });
    const combinations = [];
    const visit = (index, selected) => {
      if (index === requiredSteps.length) {
        ...
        if (total <= budget) combinations.push({ selected, unique, total, score: selected.reduce((sum, candidate) => sum + Number(candidate.score || 0), 0) });
        return;
      }
      for (const candidate of groups[requiredSteps[index]]) visit(index + 1, [...selected, candidate]);
    };
    visit(0, []);
    combinations.sort((left, right) => right.score - left.score || left.total - right.total || left.unique.map((item) => String(item.productId)).sort().join('|').localeCompare(right.unique.map((item) => String(item.productId)).sort().join('|')));
    const chosen = combinations[0];
    if (!chosen) return safeAnswer(interpretation.intent, interpretation.profile, 'El presupuesto indicado no alcanza para confirmar los pasos esenciales de una rutina sencilla.', { budget: { limit: budget, total: null, currency: 'COP', withinBudget: false } });
    ...
    return { intent: interpretation.intent, mode: 'RECOMMENDATION', message: 'Esta es una rutina sencilla confirmada dentro de tu presupuesto.', profile: interpretation.profile, routine, recommendations, productsToBuy: recommendations, budget: { limit: budget, total: finalTotal, currency: 'COP', withinBudget: true } };
  },
